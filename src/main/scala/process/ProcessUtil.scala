...
object ProcessUtil {
  ...
  def getScoresForData(sc: SparkContext, crimeData: RDD[Map[String, String]], totalCrimeScore: Double, processedPlutoData: List[Map[String, String]], getScore: String => Double): RDD[(String, Double)] = {
    val crimeScoreRDD = crimeData.map(row => processedPlutoData
      ...
      .minBy(_._2))
      .map(x => (x._1, if (inRange(x._2)) x._3 else ZERO_SCORE)).reduceByKey(_ + _).map(x => (x._1, x._2.toDouble / totalCrimeScore))
    val plutoRDD = getRDDForPlutoData(sc, processedPlutoData)
    crimeScoreRDD.fullOuterJoin(plutoRDD).mapValues(option => if (option._1.isEmpty) ZERO_SCORE else option._1.get)
  }
  ...
  private def getRDDForPlutoData(sc: SparkContext, processedPlutoData: List[Map[String, String]]) = {
    sc.parallelize(processedPlutoData).map(y => (y(BOROUGH_BLOCK), None))
  }

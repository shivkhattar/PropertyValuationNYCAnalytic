...
object ProcessUtil {
  ...
  def getScoresForData(sc: SparkContext, data: RDD[Map[String, String]], totalScore: Double,
                       processedPlutoRDD: RDD[Map[String, String]], processedPlutoData: List[Map[String, String]],
                       getScore: String => Double): RDD[(String, (Double, (String, String)))] = {
    val scoreRDD = data.map(row => processedPlutoData
      ...
      .minBy(_._2))
      .map(x => (x._1, if (inRange(x._2)) x._3 else ZERO_SCORE)).reduceByKey(_ + _).map(x => (x._1, x._2.toDouble / totalScore))

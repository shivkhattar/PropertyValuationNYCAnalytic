...
import util.EdPropConstants.{SPLIT_REGEX, UNKNOWN, PROP_BORO, PROP_BLOCK, PROP_LOT, PROP_EASEMENT, PROP_PARID, PROP_CURMKTTOT, PROP_ZIPCODE, PROP_YRBUILT, PROP_BLDG_CLASS, PROP_ZONING}

object PropertyCleanFinal extends Clean{
  ...
  def clean(sc: SparkContext, hdfs: FileSystem, inputPath: String, outputPath: String): Unit = {
    ...
    //println("FVFVRFVW")
    ///CommonUtil.getTotalCount(columnsRemoved).take(1).foreach(println)
    ...
    //println("HRELNFEWFLN:")
    //CommonUtil.getTotalCount(cleanedProperty).take(1).foreach(println)
    ...
    //CommonUtil.getTotalCount(tupledProp).take(1).foreach(println)
